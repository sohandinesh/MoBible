...
//		$(document).on('pageinit', '#files-list-page', function() {
//			Application.initFilesListPage();
//		});
//		$(document).on('pageinit', '#aurelio-page', function() {
//			Application.openLinksInApp();
//		});
//		$(document).on(
//				'pagechange',
//				function(event, properties) {
//					if (properties.absUrl === $.mobile.path
//							.makeUrlAbsolute('player.html')) {
//						Application.initPlayerPage(JSON
//								.parse(properties.options.data.file));
//					}
//				});
//		alert('Debug.Application.initApplication');
		
		Application.initPlayerPage('/android_asset/www/data/20130912e.mp3');
	},
...
//		alert('Debug.Application.initPlayerPage' + file);
//		Player.stop();
//		$('#media-played').text(file.name);
//		$('#media-path').text(file.fullPath);
		$('#media-played').text(file);
		$('#player-play').click(function() {
//			Player.playPause(file.fullPath);
			Player.playPause(file);
		});
...
//									continue;
//
//								extension = entries[i].name
//										.substr(entries[i].name
//												.lastIndexOf('.'));
//								if (entries[i].isDirectory === true
//										&& recursive === true)
//									Application.collectMedia(
//											entries[i].fullPath, recursive,
//											level + 1);
//								else if (entries[i].isFile === true
//										&& $.inArray(extension,
//												AppFile.EXTENSIONS) >= 0) {
//									appFile = new AppFile(entries[i].name,
//											entries[i].fullPath);
//									appFile.addFile();
...
//						function(error) {
//							console
//									.log('Unable to read the directory. Errore: '
//											+ error.code);
//						});
//
//		if (level === 0)
...
//			$('#' + idElement)
//					.append(
//							'<p>No files to show. Would you consider a files update (top right button)?</p>');
//			return;
//		}
//
...
//			$listElement = $('<li>');
//			$linkElement = $('<a>');
//			$linkElement.attr('href', '#').text(files[i].name).click(
//					getPlayHandler(files[i]));
//
//			// Append the link to the <li> element
//			$listElement.append($linkElement);
//
//			$linkElement = $('<a>');
//			$linkElement.attr('href', '#').text('Delete').click(
//					getDeleteHandler(files[i]));
//
//			// Append the link to the <li> element
//			$listElement.append($linkElement);
//
