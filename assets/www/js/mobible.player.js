...
//		alert('Debug.Player.initMedia ' + path);
		Player.media = new Media(path, function() {
			console.log('Media file readed succesfully');
			if (Player.media !== null)
				Player.media.release();
			Player.resetLayout();
		}, function(error) {
			navigator.notification.alert('Unable to read the media file.',
					function() {
					}, 'Error');
...
//		alert('Debug.Player.playPause');
		...
		if (Player.isPlaying === false) {
			...
			Player.mediaTimer = setInterval(function() {
				Player.media.getCurrentPosition(function(position) {
					...
					console.log('Unable to retrieve media position: '
							+ error.code);
	...
	changePlayButton : function(imageName) {
		var background = $('#player-play').css('background-image').replace(
				'url(', '').replace(')', '');

		$('#player-play').css(
				'background-image',
				'url('
						+ background.replace(/images\/.*\.png$/, 'images/'
								+ imageName + '.png') + ')');
	}
};